...
type LinksProps = {
  links: (LinkType & { visits: LinkVisit[] })[];
  totalLinks: number;
  totalPages: number;
  currentPage: number;
};
...
const Links = (
  ...
  const [orderBy, setOrderBy] = useState(
    (searchParams.orderBy as string) ?? "createdAt"
  );
  ...
  const handlePageChange = (page: number) => {
    router.push(
      `/dashboard?page=${page}&orderBy=${orderBy}&orderDirection=${orderDirection}`
    );
  };
  ...
  return (
    <>
      <div className="flex items-center justify-between gap-4">
        ...
        >
          <SelectTrigger className="w-[170px] dark:text-neutral-300">
