import { LinkVisit, UniqueLinkVisit } from "@prisma/client";
import UAParser from "ua-parser-js";
import { GeolocationAPIResponseType } from "./types";

const getGeolocationDetailsFromAPI = async (ip: string) => {
  const geolocationApiUrl = `https://api.findip.net/${ip}/?token=${process.env.GEOLOCATION_API_KEY}`;
...
const getGeolocationDetails = async (ip: string) => {
  const geolocationDetails = await getGeolocationDetailsFromAPI(ip);
...
export const aggregateVisits = (
  ...
  const clicksPerReferer: Record<string, number> = {};
  ...
  visits.forEach((visit) => {
    ...
    if (visit.referer) safeIncrement(clicksPerReferer, visit.referer);
  });
